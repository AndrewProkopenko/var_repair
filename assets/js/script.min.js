(function(){
    ...
    if (ua.search(/rv:11.0/) > 0 ) { 
        const achorsIe = document.querySelectorAll('.mobile-menu a.nav-item');
        for( let i=0; i<achorsIe.length; i++) { 
            
            let hrefIe = achorsIe[i].getAttribute('href'); 
            let newHref = hrefIe.slice(1)
            
            let scrollItem = document.getElementById(newHref);
            achorsIe[i].addEventListener('click', function(event) {  
                // console.log(newHref, scrollItem)
        ...
        for( let anchor = 0 ; anchor < anchors.length; anchor++) { 
            anchors[anchor].addEventListener("click", function(event) { 
                console.log('ach function working')
